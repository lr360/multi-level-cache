...
const createLocalCacheInvalidation = require('./create-invalidation');
const MultiError = require('./cache-lib/multi-error');

/* eslint-disable max-statements */
let MultiCache = (function() {
  /* eslint-disable max-statements */
  function MultiCache(localCache, remoteCache, localCacheInvalidation, options) {
    this.options = options != null ? options : {};

    this.useLocalCacheDefault = this.options.hasOwnProperty('useLocalCache') ? this.options.useLocalCache : true;
    this.useRemoteCacheDefault = this.options.hasOwnProperty('useRemoteCache') ? this.options.useRemoteCache : true;
    this.useLocalCacheInvalidationDefault = this.options.hasOwnProperty('useLocalCacheInvalidation') ? this.options.useLocalCacheInvalidation : true;
    ...
    this.localCacheInvalidation = typeof localCacheInvalidation === 'string' ? createLocalCacheInvalidation(localCacheInvalidation, this.options.localCacheInvalidationOptions, this.localCache) : localCacheInvalidation;
  }
  ...
  MultiCache.prototype.get = function(keys, options, callback) {
    ...
    if (self._useLocalCache(options)) {
      ...
      if (self._useRemoteCache(options)) {
  ...
  MultiCache.prototype._useLocalCacheInvalidation = function(options) {
    if (options && options.hasOwnProperty('useLocalCacheInvalidation')) {
      return options.useLocalCacheInvalidation;
    }

    return this.useLocalCacheInvalidationDefault;
  };
  ...
  MultiCache.prototype._useMethods = function(options, method) {
    ...
    if (this._useLocalCacheInvalidation(options) && this.localCacheInvalidation && this.localCacheInvalidation[method]) {
      methods.push(this.localCacheInvalidation[method]);
    }
